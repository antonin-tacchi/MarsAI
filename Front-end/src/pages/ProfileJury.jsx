  ...
  return (
    <div className="flex items-center gap-4 p-3 bg-white rounded-xl border border-[#262335]/5 hover:border-[#463699]/30 transition-colors">
      <a href={`/details-film/${film?.id ?? ""}`} className="shrink-0 w-24 h-14 rounded-lg overflow-hidden bg-[#C7C2CE]">
        <img
          src={film?.thumbnail_url ? (film.thumbnail_url.startsWith("http") ? film.thumbnail_url : `${apiUrl}${film.thumbnail_url}`) : "/placeholder.jpg"}
          alt={film?.title || ""}
...
export default function ProfileJury() {
  ...
  const fetchLists = useCallback(async () => {
    ...
    setListsError("");
    try {
      const res = await fetch(`${API_URL}/api/jury/lists`, { headers: { Authorization: `Bearer ${token}` } });
      const json = await res.json();
  ...
  useEffect(() => { if (currentView === "lists") fetchLists(); }, [currentView, fetchLists]);

  const fetchFilms = useCallback(async (p = 1, listId = null) => {
    setUiState("loading");
    setError("");
    try {
      const params = new URLSearchParams({ page: p, limit: PER_PAGE });
      if (listId) params.set("list_id", listId);
      const res = await fetch(`${API_URL}/api/jury/assigned-films?${params}`, { headers: { Authorization: `Bearer ${token}` } });
      const json = await res.json();
      if (!res.ok) throw new Error(json?.message || "Erreur");
  ...
  useEffect(() => {
    if (currentView !== "lists" && currentView.listId) fetchFilms(page, currentView.listId);
  }, [page, currentView, fetchFilms]);
  ...
  return (
    <div className="bg-[#FBF5F0] min-h-screen">
      <div className="flex flex-col gap-4 p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="bg-white rounded-2xl p-5 border border-[#262335]/5 animate-pulse">
                    ...
                    <div className="flex gap-2 mb-3"><div className="h-6 bg-[#C7C2CE] rounded-full w-16" /><div className="h-6 bg-[#C7C2CE] rounded-full w-16" /></div>
            ...
            {!listsLoading && !listsError && lists.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {lists.map((list) => {
                  ...
                  return (
                    <button key={list.id} type="button" onClick={() => handleListClick(list)}
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        ...
                        {rated > 0 && <span className="bg-[#463699] text-white text-xs font-bold px-3 py-1 rounded-full">{rated} {t("profileJury.ratedLabel")}</span>}
                        {unrated > 0 && <span className="bg-[#262335]/10 text-[#262335] text-xs font-bold px-3 py-1 rounded-full">{unrated} {t("profileJury.unratedLabel")}</span>}
                      </div>
                      <div className="w-full bg-[#262335]/10 rounded-full h-2">
                        <div className="bg-gradient-to-r from-[#463699] to-[#9a92c9] h-2 rounded-full transition-all duration-300" style={{ width: `${progress}%` }} />
                      </div>
                      <p className="text-xs text-[#262335]/50 mt-1">{progress}% {t("profileJury.completed")}</p>
                    </button>
                  );
                })}
              </div>
        ...
        {/* ════════ FILMS VIEW (inside a list) ════════ */}
        {currentView !== "lists" && (
          <>
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                <button type="button" onClick={handleBackToLists} className="flex items-center gap-2 text-[#463699] hover:text-[#262335] font-medium transition-colors">
                  <FaArrowLeft /> {t("profileJury.backToLists")}
                </button>
                <h2 className="text-xl font-bold text-[#262335]">{currentView.listName}</h2>
              </div>
              <div className="flex bg-[#262335]/10 rounded-lg p-1">
                <button type="button" onClick={() => setViewMode("grid")}
                  className={`px-3 py-1.5 rounded-md text-sm font-bold transition-colors flex items-center gap-1.5 ${viewMode === "grid" ? "bg-[#262335] text-white" : "text-[#262335] hover:bg-[#262335]/10"}`}>
                  <FaThLarge size={14} /> {t("profileJury.gridView")}
                </button>
                <button type="button" onClick={() => setViewMode("list")}
                  className={`px-3 py-1.5 rounded-md text-sm font-bold transition-colors flex items-center gap-1.5 ${viewMode === "list" ? "bg-[#262335] text-white" : "text-[#262335] hover:bg-[#262335]/10"}`}>
            ...
            <div>
                ...
                <div className="flex flex-col gap-2">
                  {[...Array(6)].map((_, i) => (
                    <div key={i} className="flex items-center gap-4 p-3 bg-white rounded-xl animate-pulse">
                      ...
                      <div className="flex-1"><div className="h-5 bg-[#C7C2CE] rounded w-1/3 mb-2" /><div className="h-4 bg-[#C7C2CE] rounded w-1/2" /></div>
                    </div>
                  ))}
                </div>
              )}
              ...
              {uiState === "error" && (
                <div className="flex flex-col items-center justify-center p-10 bg-red-50 border-2 border-red-100 rounded-[2.5rem] text-center max-w-2xl mx-auto">
                  ...
                  <Button onClick={() => fetchFilms(page, currentView.listId)}>{t("catalogs.retry")}</Button>
                </div>
              )}
              ...
              {uiState === "success" && totalPages > 1 && (
                <div className="flex justify-center items-center gap-4 mt-12 text-2xl text-[#262335]">
                  <button onClick={() => setPage(1)} disabled={page === 1} className="hover:opacity-60 disabled:opacity-30" type="button">&laquo;</button>
                  <button onClick={() => setPage((p) => Math.max(p - 1, 1))} disabled={page === 1} className="hover:opacity-60 disabled:opacity-30" type="button">&lsaquo;</button>
                  <span className="font-black tabular-nums">{page}</span>
                  <button onClick={() => setPage((p) => Math.min(p + 1, totalPages))} disabled={page === totalPages} className="hover:opacity-60 disabled:opacity-30" type="button">&rsaquo;</button>
                  <button onClick={() => setPage(totalPages)} disabled={page === totalPages} className="hover:opacity-60 disabled:opacity-30" type="button">&raquo;</button>
                </div>
              )}
            </div>
          </>
      ...
      {refuseTarget && (
        <RefuseModal film={refuseTarget} onClose={() => setRefuseTarget(null)} onConfirm={handleRefuseConfirm} t={t} />
      )}
    </div>
  );
}
