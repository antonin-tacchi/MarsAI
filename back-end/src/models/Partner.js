...
class PartnerModel {
    async getAll() {
        const [rows] = await pool.query(
            `SELECT id, name, logo, url, display_order, created_at 
             FROM partners 
    ...
    async getById(id) {
        const [rows] = await pool.query(
            `SELECT id, name, logo, url, display_order, created_at, updated_at 
    ...
    async create(data) {
        const { name, logo, url, display_order = 0 } = data;
        const [result] = await pool.query(
            `INSERT INTO partners (name, logo, url, display_order) VALUES (?, ?, ?, ?)`,
            [name, logo, url, display_order]
        );
        return { id: result.insertId, name, logo, url, display_order };
    }

    async update(id, data) {
        const { name, logo, url, display_order } = data;
        const updates = [];
        const values = [];
        ...
        values.push(id);
        const [result] = await pool.query(
            `UPDATE partners SET ${updates.join(", ")} WHERE id = ?`, values
        );
