...
export default class JuryAssignment {
  ...
  /**
   * Insère un lot d'assignations en une seule requête.
   * @param {Array<{filmId: number, juryId: number}>} assignments
   * @param {number} assignedBy - ID du super jury qui génère
   */
  static async bulkCreate(assignments, assignedBy) {
    if (!assignments.length) return;

    const values = assignments.map((a) => [a.juryId, a.filmId, assignedBy]);
    ...
    await db.query(sql, flat);
  }

  /** Supprime toutes les assignations existantes. */
  static async clearAll() {
    await db.query("DELETE FROM jury_assignments");
  }

  /** Récupère les notes existantes sous forme de Map<filmId, Set<juryId>>. */
  static async getExistingRatingsMap() {
    const [rows] = await db.query(
      "SELECT film_id, user_id FROM jury_ratings"
    );

    const map = new Map();
    for (const row of rows) {
      if (!map.has(row.film_id)) {
        map.set(row.film_id, new Set());
      }
      map.get(row.film_id).add(row.user_id);
    }
    return map;
  }

  /** Récupère tous les jurys (rôle 1). */
  static async getAllJuries() {
    const [rows] = await db.query(`
  ...
  /** Stats globales de la répartition actuelle. */
  static async getDistributionOverview() {
    const [rows] = await db.query(`
      SELECT
        u.id AS jury_id,
        u.name AS jury_name,
        u.email AS jury_email,
        COUNT(ja.film_id) AS film_count,
        SUM(CASE WHEN jr.id IS NOT NULL THEN 1 ELSE 0 END) AS rated_count
      FROM users u
      INNER JOIN user_roles ur ON ur.user_id = u.id AND ur.role_id = 1
      LEFT JOIN jury_assignments ja ON ja.jury_id = u.id
      LEFT JOIN jury_ratings jr ON jr.film_id = ja.film_id AND jr.user_id = u.id
      GROUP BY u.id, u.name, u.email
      ORDER BY u.id
    `);

    const [totalFilms] = await db.query("SELECT COUNT(*) AS cnt FROM films");
    const [totalAssignments] = await db.query(
      "SELECT COUNT(*) AS cnt FROM jury_assignments"
    );

    return {
      juries: rows,
      totalFilms: totalFilms[0].cnt,
      totalAssignments: totalAssignments[0].cnt,
    };
  }
}
